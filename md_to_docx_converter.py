...
def set_korean_font(run):
    """한글과 영문을 모두 지원하는 폰트 설정"""
    try:
        # 한글 폰트 설정 (윈도우 호환)
        run._element.rPr.rFonts.set(qn('w:eastAsia'), '맑은 고딕')  # 한글용
        run._element.rPr.rFonts.set(qn('w:ascii'), 'Arial')      # 영문용  
        run._element.rPr.rFonts.set(qn('w:hAnsi'), 'Arial')     # 서구 문자용
        run._element.rPr.rFonts.set(qn('w:cs'), '맑은 고딕')     # 복합 스크립트용
    except:
        # 폴백: 기본 폰트만 설정
        run.font.name = 'Arial'

def set_korean_font_for_style(font_obj):
    """스타일 폰트 객체에 한글/영문 폰트 설정"""
    try:
        # 기본 폰트명 설정 (윈도우 호환)
        font_obj.name = 'Malgun Gothic'
        
        # XML 직접 조작으로 세부 폰트 설정
        font_element = font_obj._element
        
        # 기존 rFonts 요소가 있으면 제거
        for rFonts in font_element.xpath('.//w:rFonts', namespaces={'w': 'http://schemas.openxmlformats.org/wordprocessingml/2006/main'}):
            rFonts.getparent().remove(rFonts)
        
        # 새 rFonts 요소 생성
        rFonts = OxmlElement('w:rFonts')
        rFonts.set(qn('w:ascii'), 'Arial')         # 영문
        rFonts.set(qn('w:eastAsia'), 'Malgun Gothic')  # 한글  
        rFonts.set(qn('w:hAnsi'), 'Arial')        # 서구문자
        rFonts.set(qn('w:cs'), 'Malgun Gothic')       # 복합스크립트
        font_element.insert(0, rFonts)
...
class FootnoteManager:
    ...
    def __init__(self, document: Document):
        ...
        self.has_footnote_section_in_md = False  # MD에 주석 섹션이 있는지 추적
    ...
    def add_footnotes_section(self):
        ...
        # 마크다운에 이미 주석 섹션이 있으면 제목 추가하지 않음
        if not self.has_footnote_section_in_md:
            # 주석 섹션 제목
...
class DocumentStyler:
    ...
    def setup_styles(self):
        ...
        if 'CoverSubtitle' not in styles:
            ...
            cover_subtitle_font.name = 'Arial'
        ...
        if 'CustomHeading2' not in styles:
            ...
            h2_style.paragraph_format.left_indent = Inches(0.1)
        ...
        if 'CustomHeading3' not in styles:
            ...
            h3_style.paragraph_format.left_indent = Inches(0.2)
        ...
        # 본문 1단계 들여쓰기 스타일 (글머리 없는 일반 텍스트용)
        if 'CustomBodyIndent1' not in styles:
            body_indent1_style = styles.add_style('CustomBodyIndent1', WD_STYLE_TYPE.PARAGRAPH)
            body_indent1_font = body_indent1_style.font
            body_indent1_font.name = 'Arial'
            body_indent1_font.size = Pt(11)
            body_indent1_style.paragraph_format.line_spacing = 1.3
            body_indent1_style.paragraph_format.space_after = Pt(6)
            body_indent1_style.paragraph_format.left_indent = Inches(0.3)  # □ 다음 컨텍스트
            body_indent1_style.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
        
        # 본문 2단계 들여쓰기 스타일 (글머리 없는 일반 텍스트용)
        if 'CustomBodyIndent2' not in styles:
            body_indent2_style = styles.add_style('CustomBodyIndent2', WD_STYLE_TYPE.PARAGRAPH)
            body_indent2_font = body_indent2_style.font
            body_indent2_font.name = 'Arial'
            body_indent2_font.size = Pt(11)
            body_indent2_style.paragraph_format.line_spacing = 1.3
            body_indent2_style.paragraph_format.space_after = Pt(6)
            body_indent2_style.paragraph_format.left_indent = Inches(0.6)  # ○ 다음 컨텍스트
            body_indent2_style.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
        
        # 본문 3단계 들여쓰기 스타일 (글머리 없는 일반 텍스트용)
        if 'CustomBodyIndent3' not in styles:
            body_indent3_style = styles.add_style('CustomBodyIndent3', WD_STYLE_TYPE.PARAGRAPH)
            body_indent3_font = body_indent3_style.font
            body_indent3_font.name = 'Arial'
            body_indent3_font.size = Pt(11)
            body_indent3_style.paragraph_format.line_spacing = 1.3
            body_indent3_style.paragraph_format.space_after = Pt(6)
            body_indent3_style.paragraph_format.left_indent = Inches(0.9)  # - 또는 • 다음 컨텍스트
            body_indent3_style.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
        ...
        # □ 아래 대시 스타일 (○보다 조금 더 들여쓰기)
        if 'CustomListLevel2_5' not in styles:
            level2_5_style = styles.add_style('CustomListLevel2_5', WD_STYLE_TYPE.PARAGRAPH)
            level2_5_font = level2_5_style.font
            level2_5_font.name = 'Arial'
            level2_5_font.size = Pt(11)
            level2_5_font.color.rgb = RGBColor(0x00, 0x00, 0x00)
            level2_5_style.paragraph_format.left_indent = Inches(0.6)  # ○(0.5) 보다 조금 더
            level2_5_style.paragraph_format.space_after = Pt(3)
            level2_5_style.paragraph_format.line_spacing = 1.2
        ...
        # ①②③ 숫자 글머리 스타일들 (상위 요소별로 다른 들여쓰기)
        # □ 아래 ①②③
        if 'CustomNumberedLevel1' not in styles:
            num1_style = styles.add_style('CustomNumberedLevel1', WD_STYLE_TYPE.PARAGRAPH)
            num1_font = num1_style.font
            num1_font.name = 'Arial'
            num1_font.size = Pt(11)
            num1_font.color.rgb = RGBColor(0x00, 0x00, 0x00)
            num1_style.paragraph_format.left_indent = Inches(0.4)  # □(0.25) 보다 조금 더
            num1_style.paragraph_format.space_after = Pt(3)
            num1_style.paragraph_format.line_spacing = 1.2
        
        # ○ 아래 ①②③
        if 'CustomNumberedLevel2' not in styles:
            num2_style = styles.add_style('CustomNumberedLevel2', WD_STYLE_TYPE.PARAGRAPH)
            num2_font = num2_style.font
            num2_font.name = 'Arial'
            num2_font.size = Pt(11)
            num2_font.color.rgb = RGBColor(0x00, 0x00, 0x00)
            num2_style.paragraph_format.left_indent = Inches(0.65)  # ○(0.5) 보다 조금 더
            num2_style.paragraph_format.space_after = Pt(3)
            num2_style.paragraph_format.line_spacing = 1.2
        
        # - 아래 ①②③
        if 'CustomNumberedLevel3' not in styles:
            num3_style = styles.add_style('CustomNumberedLevel3', WD_STYLE_TYPE.PARAGRAPH)
            num3_font = num3_style.font
            num3_font.name = 'Arial'
            num3_font.size = Pt(11)
            num3_font.color.rgb = RGBColor(0x00, 0x00, 0x00)
            num3_style.paragraph_format.left_indent = Inches(0.9)  # -(0.75) 보다 조금 더
            num3_style.paragraph_format.space_after = Pt(3)
            num3_style.paragraph_format.line_spacing = 1.2
...
class DocxConverter:
    ...
    def __init__(self):
        ...
        self.in_footnote_section = False  # 주석 섹션 내부 여부
        self.table_counter = 0   # 표 번호
        self.processed_captions = set()  # 이미 처리된 캡션 추적
        
        # 문서 기본 폰트 설정 (윈도우 호환)
        self._set_document_default_font()
    ...
    def _set_document_default_font(self):
        """문서 전체 기본 폰트 설정 (윈도우 한글 호환)"""
        try:
            # 문서의 기본 스타일 가져오기
            styles = self.document.styles
            ...
            font.name = 'Malgun Gothic'  # 기본 폰트
            
            # XML 레벨에서 세부 폰트 설정
            style_element = default_style._element
            font_elements = style_element.xpath('.//w:rFonts', 
                namespaces={'w': 'http://schemas.openxmlformats.org/wordprocessingml/2006/main'})
            
            if font_elements:
                rFonts = font_elements[0]
            else:
                # rFonts 요소 새로 생성
            ...
            # 각 문자 체계별 폰트 설정
            rFonts.set(qn('w:ascii'), 'Arial')         # 영문
            rFonts.set(qn('w:eastAsia'), 'Malgun Gothic')  # 한글/중문/일문
            rFonts.set(qn('w:hAnsi'), 'Arial')        # 서구 문자
            rFonts.set(qn('w:cs'), 'Malgun Gothic')       # 복합 스크립트
            
        except Exception as e:
            print(f"기본 폰트 설정 중 오류: {e}")
    ...
    def _add_heading(self, element):
        ...
        # 주석 섹션 감지
        if text.strip() == '주석' and level == 2:
            self.in_footnote_section = True
            self.footnote_manager.has_footnote_section_in_md = True  # MD에 주석 섹션이 있음을 표시
        elif level <= 2 and text.strip() != '주석':
            self.in_footnote_section = False
    ...
    def _add_intelligent_paragraph(self, element):
        ...
        # 숫자로 시작하는 중제목 (1. 2. 3.) - 주석 섹션에서는 일반 본문으로 처리
        elif re.match(r'^\d+\.\s+', text) and not self.in_footnote_section:
            para = self.document.add_paragraph(text)
    ...
    def _process_cell_text_with_bold(self, paragraph, text: str, is_header: bool):
        """테이블 셀 내 Bold 마크다운 구문 처리"""
        bold_pattern = r'\*\*(.*?)\*\*'
    ...
    def _process_markdown_directly(self, md_content: str):
        ...
        current_table = []
        in_table = False
        last_bullet_level = None  # 마지막 글머리 기호 레벨 추적
        ...
        while i < len(lines):
            ...
            # 계층적 번호 체계 처리
                ...
                last_bullet_level = self._get_hierarchical_level(line)
            # 로마숫자로 시작하는 대제목 (Ⅰ, Ⅱ, Ⅲ...)
            ...
            # 특수 번호 (①②③)
                ...
                # 상위 컨텍스트에 따른 적절한 스타일 결정
                numbered_style = self._get_numbered_style_for_context(last_bullet_level)
                para = self.document.add_paragraph(line)
                para.style = numbered_style
            # 일반 텍스트
            else:
                # LaTeX 수식 처리
                ...
                # 주석이 포함된 텍스트 처리
                if '^' in line and self._has_footnote_pattern(line):
                    # 컨텍스트에 따른 들여쓰기 스타일 적용 (대시 없이)
                    indented_style = self._get_indented_style_for_context(last_bullet_level)
                    self._add_paragraph_with_footnotes_and_style(line, indented_style)
                else:
                    # 일반 텍스트는 컨텍스트에 따른 들여쓰기만 적용 (대시 없이)
                    indented_style = self._get_indented_style_for_context(last_bullet_level)
                    # 마크다운 포매팅이 있는지 확인하고 처리
                    if '**' in line or '*' in line:
                        self._add_paragraph_with_formatting_and_style(line, indented_style)
                    else:
                        para = self.document.add_paragraph(line)
                        para.style = indented_style
    ...
    def _should_add_dash_bullet(self, line: str) -> bool:
        """일반 텍스트에 대시 글머리를 자동 추가할지 판단"""
        # 들여쓰기가 있으면 추가하지 않음 (의도적 들여쓰기는 글머리 체계 준수)
        if line.startswith('  '):
            return False
        
        # 이미 글머리 기호가 있으면 추가하지 않음
        if line.startswith(('□', '○', '-', '•', '①', '②', '③', '④', '⑤')):
            return False
        
        # 헤딩이나 특수 형식이면 추가하지 않음
        if line.startswith(('#', '**', '<', '|', '---')):
            return False
            
        # 숫자로 시작하는 번호 체계면 추가하지 않음
        if re.match(r'^\d+[\.\)]\s+', line):
            return False
            
        # 로마숫자로 시작하면 추가하지 않음
        if re.match(r'^[ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩⅪⅫⅩⅢⅩⅣⅩⅤⅩⅥⅩⅦⅩⅧⅩⅨⅩⅩ]+\s+', line):
            return False
        
        # 짧은 텍스트 (제목이나 캡션일 가능성)는 추가하지 않음
        if len(line.strip()) < 8:
            return False
        ...
        # 다음으로 끝나는 문장들에만 대시 추가 (글머리 체계 준수)
        endings = ('.', '!', '?', '함', '됨', '임', '다', '며', '고', '니다', '됩니다', '합니다', '있습니다', '있음', '같음', '나타남', '보임')
        if any(line.strip().endswith(ending) for ending in endings):
            # 주석이 있는 텍스트도 허용
            if re.search(r'\^[\d]+\^', line):
                return True
            # 한글/영문으로 시작하는 경우만
            if re.match(r'^[가-힣a-zA-Z]', line.strip()):
                return True
            
        return False
    
    def _get_hierarchical_level(self, line: str) -> int:
        """계층적 번호 체계의 레벨 반환"""
        # 1. 형태 = 레벨 1 (□과 동등)
        if re.match(r'^\d+\.\s+', line):
            return 1
    ...
    def _get_dash_style_for_context(self, last_bullet_level) -> str:
        """상위 컨텍스트에 따른 대시(-) 스타일 결정"""
        if last_bullet_level is None:
            # 컨텍스트가 없으면 기본 대시 레벨
            return 'CustomListLevel3'
        elif last_bullet_level == 1:  # □ 다음 또는 1. 다음
            # □ 아래에는 ○보다 조금 더 들여쓰기하는 새로운 스타일
            return 'CustomListLevel2_5'  # ○(0.5) 보다 조금 더(0.6)
        elif last_bullet_level == 2:  # ○ 다음 또는 1.1 다음
            # ○ 아래에는 현재 - 레벨이 맞음
            return 'CustomListLevel3'  # 원래 대시 레벨(0.75)
        elif last_bullet_level >= 3:  # - 또는 • 다음, 또는 1.1.1 다음
            # - 아래에는 • 레벨
            return 'CustomListLevel4'  # • 레벨(1.0)
        else:
            return 'CustomListLevel3'
    
    def _get_numbered_style_for_context(self, last_bullet_level) -> str:
        """상위 컨텍스트에 따른 ①②③ 숫자 글머리 스타일 결정"""
        if last_bullet_level is None:
            # 컨텍스트가 없으면 기본 숫자 레벨
            return 'CustomList'  # 기존 스타일 유지
        elif last_bullet_level == 1:  # □ 다음 또는 1. 다음
            # □ 아래에는 □보다 조금 더 들여쓰기
            return 'CustomNumberedLevel1'  # 0.4 inches
        elif last_bullet_level == 2:  # ○ 다음 또는 1.1 다음
            # ○ 아래에는 ○보다 조금 더 들여쓰기
            return 'CustomNumberedLevel2'  # 0.65 inches
        elif last_bullet_level >= 3:  # - 또는 • 다음, 또는 1.1.1 다음
            # - 아래에는 -보다 조금 더 들여쓰기
            return 'CustomNumberedLevel3'  # 0.9 inches
        else:
            return 'CustomList'
    ...
    def _get_indented_style_for_context(self, last_bullet_level) -> str:
        ...
        if last_bullet_level is None:
            ...
            # □ 아래에는 □보다 조금 더 들여쓰기 (0.3 inches)
            return 'CustomBodyIndent1'
        elif last_bullet_level == 2:  # ○ 다음 또는 1.1 다음
            # ○ 아래에는 ○보다 조금 더 들여쓰기 (0.6 inches)
            return 'CustomBodyIndent2'
        elif last_bullet_level >= 3:  # - 또는 • 다음, 또는 1.1.1 다음
            # - 아래에는 -보다 조금 더 들여쓰기 (0.9 inches)
            return 'CustomBodyIndent3'
        else:
            return 'CustomBody'
    
    def _add_paragraph_with_formatting_and_style(self, text: str, style_name: str):
        """마크다운 포매팅이 포함된 단락을 특정 스타일로 추가"""
        para = self.document.add_paragraph()
        para.style = style_name
        self._add_formatted_text_to_paragraph(para, text, Pt(11))
    
    def _add_paragraph_with_footnotes_and_style(self, text: str, style_name: str):
        """주석이 포함된 텍스트를 특정 스타일로 추가"""
        para = self.document.add_paragraph()
        para.style = style_name
        ...
        for match in re.finditer(footnote_pattern, text):
            ...
            # 주석 추가
            self.footnote_manager.add_footnote(para, "", footnote_content)
            ...
            for j, cell_text in enumerate(cells):
                if j < cols:
                    ...
                    if self._has_footnote_pattern(cell_text):
                        ...
                        # 마크다운 Bold 구문 처리
                        self._process_cell_text_with_bold(paragraph, cell_text, i == 0)
